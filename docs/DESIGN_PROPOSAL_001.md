...
- **Category-colour accents** — magenta vocals, amber melody, green bass, blue drums (see §3.3 / §6 for the ColorUtil reconciliation).
...
| Category         | **Target direction** (approved reference §2 / §8.3) | Current ColorUtil fill (`-700`) | Current text tint (`-300`) | Icon meaning (icon system — §3.11)      |
| ---------------- | --------------------------------------------------- | ------------------------------- | -------------------------- | --------------------------------------- |
| **Vocals (Vox)** | **magenta / pink** ← restyle from red               | `#b91c1c` (red-700, _today_)    | `#fca5a5`                  | Vintage / clear **microphone**          |
| **Melody**       | **amber / orange**                                  | `#a16207` (yellow-700)          | `#fcd34d`                  | **Treble clef**                         |
...
> **Category-hue direction:** the **Target direction** column is what the approved reference (§2) requires — magenta Vocals, amber Melody, green Bass, blue Drums. The **Current ColorUtil** columns are today's `-700`/`-300` values, used so this document's diagrams render; the **Vocals red→magenta and Melody yellow→amber restyle** is decided at the §8.3 palette sign-off and applied in `ColorUtil` (the single source of truth), then **re-verified against the physical pillar LEDs** before finalising. Whichever final hues are chosen keep the **saturated fill / lighter text** dual-role for AA (§7.2).
>
> **Icon rule:** the four category meanings above must be **immediately recognisable** and drawn from **one coherent icon system** (matching stroke weight, detail, optical scale, circular medallion presentation) — see the icon strategy in **§3.11**. **No arbitrary occult symbol** may stand in for a music category, and **no hand-authored primitive path** (like the placeholder glyphs in this doc's diagrams) may ship as a final icon.
>
...
**Category colour direction (from the approved reference §2): magenta Vocals · amber Melody · green Bass · blue Drums.** `ColorUtil` stays the **single source of truth**; `ColorUtil.getBackgroundColorFromType` currently maps Vox→`bg-red-700`, Melody→`bg-yellow-700`, Bass→`bg-green-700`, Drums→`bg-blue-700`. To match the reference, Vox should move **red → magenta/pink** and Melody **yellow → amber/orange** (Bass/Drums already align). Per PRD F4 this is a **deliberate restyle of the values inside that one function** — recorded here as the single place it is decided, to be applied in the implementation, **and the exact final hues must be confirmed against the physical pillar LED colours before finalising** (PROJECT_BRIEF). The surrounding page/gold/ink palette is open. Three directions:
...
Whatever is chosen must keep **AA data contrast** (§7.2) and keep the **fill / lighter-text** dual-role (a `-700`-style saturated fill for medallions/LED-match, a lighter tint for any category text). Because the category direction is now set to the reference's **magenta/amber/green/blue**, the ColorUtil restyle above applies under **any** page-palette option. Recommendation to consider: **Option C ("Candlelit Parchment")** or **A ("Obsidian & Gilt")** — both sit closest to the reference's warm black-and-gold engraving; whichever is picked, re-verify category hues against the LEDs. **Artists decide.**
