...
- **Mode taxonomy renamed (human, 2026-07-15 — propagation pending):** the three modes are now **play / tutorial / DJ** (replacing normal / dj / debug). **Debug is no longer a mode** — it becomes a diagnostics **panel** that can be shown in any mode. **Tutorial is a new, as-yet-undesigned mode** (no requirements exist; needs PRD definition before any implementation). WOW-007A implements the **play-mode** screen under the new name. ⚠️ Follow-up needed: propagate the rename through ADR-003, ADR-005, ADR-006, PRD F6/FR1/FR3, UX_UI_PRINCIPLES, and DESIGN_PROPOSAL_001 §6 (docs ticket / `/sync-docs`), and design the tutorial mode.
- **Per-mode URL routes (human, 2026-07-15 — deferred):** each mode should have its own **bookmarkable, reloadable route** (open-able by an existing external bash script). This **supersedes ADR-005's "no separate views to route between"** and needs an ADR-005 amendment + its own follow-up ticket (routing approach + possible router-dependency approval). Deliberately **not** built into WOW-007A; the spike uses a temporary `#play-spike` hash demo switch only.
