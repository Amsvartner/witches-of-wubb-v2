...
| Nit 1 — `cors: { origin: true }`                                  | **Fixed.** `sim/server.ts:74` now restricts CORS to `/^https?:\/\/(localhost                                                                                                                                                                                                                                                    | 127\.0\.0\.1)(:\d+)?$/`with a comment noting the real backend allows any origin (sim-only tightening, not a contract behavior). Combined with the`127.0.0.1` bind, the network surface is now localhost-only at both layers. Coordinator reports a live re-verification (UI reconnect + full-spell scenario) after the change. | yes |
