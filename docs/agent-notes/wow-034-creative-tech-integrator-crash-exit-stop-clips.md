...
Copilot (0 comments), audio-ableton-reviewer (APPROVE), general reviewer (APPROVE-WITH-NITS), and hardware-safety-reviewer (APPROVE) all reviewed PR #40 independently, in parallel. No Blocking findings from anyone — the crash-exit bound, guard, and re-entrancy logic all held up under independent tracing, including the hardware-safety-reviewer reading `ableton-js`'s actual installed source rather than trusting this note's characterization of it.

That independent read caught one real inaccuracy in this note (and in `backend/index.ts`'s comment): investigation point 1 below originally claimed `ableton-js`'s own per-command timeout means "an individual command cannot hang indefinitely even in the worst case." The hardware-safety-reviewer found that's not quite true — `ableton-js`'s `handleDisconnect` cancels in-flight commands' timeouts on disconnect without ever rejecting them, a real gap in the dependency itself. It doesn't change the safety conclusion (the outer `Promise.race` in `crashExit` bounds against an independent timer regardless of whether `ableton-js` ever settles anything), but the _stated reason_ was wrong. Corrected both the code comment and point 1 below in place, credited to that review. Not re-spawning the three reviewers for this round: the fix is comment/doc text only, touches no logic or Ableton call site, and all three explicitly called this finding non-blocking / verdict-unaffected.

Two other non-blocking observations converged on independently by multiple reviewers, deliberately **not** fixed here (out of this ticket's stated scope, per all three): `main()`'s startup-failure path (`backend/index.ts`, the `main().catch(...)` at the bottom of the file) still doesn't attempt a stop, including in the narrow case where `tracks` was already populated before a _later_ startup step throws. Low real-world likelihood (no tag can have been processed yet at that point in boot) and suggested by reviewers as a fold-in for WOW-032 rather than a fix here — noted in the PR body for the human, not actioned.

Full review notes: `docs/agent-notes/wow-034-audio-ableton-reviewer-signoff.md`, `docs/agent-notes/wow-034-hardware-safety-reviewer-signoff.md`, `docs/agent-notes/wow-034-reviewer-verdict.md`.
...
1. **Can `ableton-js`'s `sendCommand` hang forever if Ableton is unresponsive or the crash is Ableton-connection-related?** Checked the pinned version (`ableton-js@3.1.5`, resolved via `backend/yarn.lock`) against its upstream source at tag `v3.1.5`. `sendCommand(command, timeout = 2000)` has a built-in 2000ms default timeout implemented via `setTimeout` + `Promise` rejection (`TimeoutError`) for the common case. Transport is UDP on localhost (`dgram`, `udp4`).

   **Correction (hardware-safety-reviewer sign-off, PR #40):** the claim above — "an individual command cannot hang indefinitely even in the worst case" — is not quite right, and the review caught it by reading the installed source rather than trusting this note. `Ableton.prototype.handleDisconnect` cancels every in-flight command's timeout on a disconnect event without ever rejecting it (`this.msgMap.forEach((msg) => msg.clearTimeout()); this.msgMap.clear();`), so a command in flight at the exact moment of a disconnect hangs forever at the `ableton-js` layer — a real gap in the dependency itself. This doesn't change the safety conclusion: `crashExit`'s outer `Promise.race` in `backend/index.ts` bounds against a bare `setTimeout` that doesn't depend on `ableton-js` ever settling anything, so the ~1.5s bound holds regardless. But it means the _reason_ this is safe is the outer independent race, not "the library's own timeout is watertight" — the code was already written the safer way (see point 2), this note's stated rationale for it just overclaimed. `backend/index.ts`'s comment above `CRASH_EXIT_STOP_TIMEOUT_MS` has been corrected to match.
