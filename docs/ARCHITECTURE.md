...
- `src/assets/Music Database.csv` — the RFID→clip mapping (production data; read by the **backend** at startup via relative path, and imported by the frontend via `src/lib/database-output.ts`).
