...
- Status: ✅ **Done — `docs/DESIGN_PROPOSAL_001.md` merged in PR #51 (2026-07-15).** Outcomes that supersede parts of the original ask below: the concept image is **no longer "inspiration only"** — `docs/design/hexology-grimoire-concept-3.png` (gold-slider variant) is the **approved primary visual reference / quality bar**; the first-round SVG mockup was rejected and removed; **Fondamento rejected** (typography → 3 options + specimen); improvised icons rejected (conventional **mic/treble-clef/bass-clef/drum-kit** set chosen); **Vocals = red per spec** (reference's pink does not apply), Bass green / Drums blue align, **Melody hue open**; **visible Help/Settings** with mode access via a Settings modal (**ADR-006 amended 2026-07-15**); the **pillar border/frame is an open choice** (proposal §8.5). Remaining human visual sign-offs (typography, page palette, pillar frame, Melody hue, mode-access variant, F3) live in `DECISIONS_NEEDED.md` + proposal §8 and are gated behind the **WOW-007A** visual-fidelity spike before any broader UI implementation.
...
- Design authority & inputs:
  ...
  - **The generated concept images are visual-language inspiration only** — mood, colour, atmosphere, ceremonial identity. They are not layout specs or pixel targets. _[Superseded 2026-07-15 — see Status: `hexology-grimoire-concept-3.png` is now the approved primary visual reference and quality bar, not mere inspiration; the wireframe still governs layout.]_
...
- Description: For the canonical 1024×1280 portrait viewport: visitor display with **category icon + category name per pillar** (no song/picture names — PRD F3), **category legend** (F4, colors from `src/util/ColorUtil.ts`: Vox red-700, Bass green-700, Drums blue-700, Melody yellow-700 _[Melody amber-vs-yellow now an open sign-off — DECISIONS_NEEDED / proposal §8.3]_), grimoire-extension background, no recipes/spell names (F5). **Main-screen modes** (ADR-003 amended 2026-07-11; ADR-006 amended 2026-07-11 & **2026-07-15** — mode access via a visible Settings modal, superseding hidden-gesture-only; supersedes the earlier page-vs-overlay exploration): design all three modes — **normal** (visitor experience; tempo/volume/key controls stay visible), **dj** (extended controls beside each pillar incl. per-pillar clip selection, moved out of the old debug panel), **debug** (small bottom panel: API/socket-event log, versions, connection state — no performance controls).
