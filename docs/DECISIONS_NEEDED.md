...
- **Category colours:** **Vocals = red** per the original spec — the reference's pink does **not** apply (resolved 2026-07-15, [Requirements over reference]); Bass green / Drums blue align with `ColorUtil`. **Melody resolved 2026-07-15/17 (human): a warm yellow — `ColorUtil` restyled `bg-yellow-700` → `bg-yellow-600`, then to the custom `bg-melody-yellow` token (`#dfa50a`) in WOW-007A.** ⚠️ Still to do: **re-verify the warm yellow against the physical pillar LEDs** before installation use (PROJECT_BRIEF).
- **Tutorial-mode design (open):** the 2026-07-15 mode-taxonomy rename — three modes now **play / tutorial / DJ**, with **debug demoted to a diagnostics panel** shown in any mode — has been **propagated** into ADR-003/005/006, PRD F6/FR1/FR3, UX_UI_PRINCIPLES, and DESIGN_PROPOSAL_001 §6 (`/sync-docs`, 2026-07-17). WOW-007A shipped the **play-mode** screen (merged, PR #53). ⚠️ **Still open:** **tutorial** is a named-but-undesigned mode — no requirements or design exist; it needs a **PRD definition before any implementation** (project-manager).
- **Per-mode URL routes (human, 2026-07-15 — deferred, open):** each mode should have its own **bookmarkable, reloadable route** (open-able by an existing external bash script). This **supersedes ADR-005's "no separate views to route between"** — **ADR-005 amended** to record it (`/sync-docs`, 2026-07-17). ⚠️ **Still open:** the routing **approach**, **possible router-dependency approval**, and implementation — its **own follow-up ticket** (project-manager). Deliberately **not** built into WOW-007A; the spike uses a temporary `#play-spike` hash demo switch only.
- **Mode access via visible Settings modal:** **ADR-006 amended 2026-07-15 (done)** — mode switching moved to a visible Settings modal, superseding hidden-gesture-only (`DESIGN_PROPOSAL_001.md` §6/§8.1; propagated to ADR-003, PRD F6/FR1, UX_UI_PRINCIPLES). **Still open (§8.1):** the access variant — A visible-only / B +covert gesture / C gated; if B, the "which themed element hosts the gesture" question reopens (§3.11 hidden-gesture asset row).
- **Queued sample names vs PRD F3 (needs confirmation):** the human-directed WOW-007A play-mode design shows **queued sample names** (up to two rows, each with play + remove controls) on the visitor-facing play screen; PRD **F3 currently bars song/sample names from the visitor display**. Confirm the F3 amendment (names allowed in play mode) or restrict the queued rows to state-only before production cutover. Related: the earlier "Currently-playing vs PRD F3" question below.
...
- The current design has one structural fragility WOW-030 documents but doesn't remove: pillar identity is inferred entirely from which static IP a UDP packet arrives from. If a device is flashed with the wrong `PILLAR_IP`, physically swapped between pillars without reflashing, or a device's IP is ever misconfigured post-deployment, the backend silently can't place that device's tags (logged as an unknown-IP warning naming the offending address, and ignored — WOW-017, now merged on main), and either way nothing is surfaced visibly to an operator mid-show. Carrying the pillar id explicitly in the payload would make the reader itself the single source of truth for its own identity, removing the IP-matching indirection entirely.
...
- Melody hue (§8.3): **warm yellow** — `ColorUtil` `bg-yellow-700` → `bg-yellow-600`, then refined 2026-07-17 to the custom `melody-yellow` token (`#dfa50a`, clearly yellow to match the physical pillar but still warm); physical-LED re-verification still pending → src/util/ColorUtil.ts, tailwind.config.cjs
- Drums hue (2026-07-17, human): **desaturated** from blue-700 (`#1d4ed8`, read far more intense than the other categories) to the custom `drums-blue` token (`#3559c0`); physical-LED re-verification pending alongside Melody → src/util/ColorUtil.ts, tailwind.config.cjs
- Pillar frames (2026-07-17, human): frames carry the **category colour** for wayfinding (empty pillars a low-key dark amber) with a discrete category-tinted gradient wash on the card; pillar names removed — the playing sample's category heads the card → src/component/PillarFrame.tsx, PillarCard.tsx
...
- Mode taxonomy: **play / tutorial / DJ, debug demoted to a diagnostics panel** — **propagated 2026-07-17 (`/sync-docs`)** into ADR-003 (amendment), ADR-005/006 (notes), docs/adr/README.md, PRD F6/FR1/FR3, UX_UI_PRINCIPLES, DESIGN_PROPOSAL_001 §6; only **tutorial-mode design** remains open (above) → docs/adr/003·005·006, docs/PRD.md, docs/UX_UI_PRINCIPLES.md, docs/DESIGN_PROPOSAL_001.md
- Per-mode URL routes wanted — **ADR-005 amended 2026-07-17 (`/sync-docs`)** to record the supersession; routing approach + possible router dependency + implementation deferred to a follow-up ticket (open, above) → docs/adr/005-hand-rolled-routing.md
