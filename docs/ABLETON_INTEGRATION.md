...
- `ableton-js` 3.1.5 bridges to a running Ableton Live set (requires the ableton-js MIDI remote script installed in Live — see README link). Discovery happens via `$TMPDIR/ableton-js-*.port` files; if the backend hangs at startup because macOS purged the server port file, run `yarn fix-ableton-port` (see README "Troubleshooting: backend hangs or exits at startup").
